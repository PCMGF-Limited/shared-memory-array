...
class TestManagedSharedArrayBasic:
    ...
    def test_managed_returns_managedsharedarray_instance(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedMemoryArray.allocate(manager, shape=(10,), dtype=np.dtype(np.float64))
            managed = sa.managed()
...
class TestManagedSharedArrayNesting:
    ...
    def test_nested_manager_and_array_contexts(self):
        ...
        with SharedMemoryManager() as manager:
            with SharedMemoryArray.allocate(manager, (100,), np.float32).managed() as sa:
                arr = sa.as_array()
                arr[:] = 99.9
...
class TestManagedSharedArrayCleanupBehavior:
    ...
    def test_cleanup_with_different_exception_types(self):
        ...
        for exc in exception_types:
            with SharedMemoryManager() as manager:
                sa = SharedMemoryArray.allocate(manager, shape=(5,), dtype=np.int64)
                name = sa.name
                ...
                with pytest.raises(FileNotFoundError):
                    SharedMemoryArray.attach(name=name, shape=(5,), dtype=np.int64)
...
class TestNameProperty:
    ...
    def test_name_uniqueness_across_allocations(self):
        ...
        with SharedMemoryManager() as manager:
            names = set()
            for _ in range(10):
                sa = SharedMemoryArray.allocate(manager, shape=(10,), dtype=np.uint8)
                names.add(sa.name)
    ...
    def test_attach_by_name_works(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            # Attach using name
            client = SharedMemoryArray.attach(name=name, shape=(10,), dtype=np.float64)
...
class TestSafeVsUnsafeAllocate:
    ...
    def test_allocate_validates_shape_unsafe_does_not(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            with pytest.raises((ValueError, OSError)):
                sa_unsafe = SharedMemoryArray.allocate_unsafe(manager, shape=(-10,), dtype=np.float64)
    ...
    def test_allocate_performance_overhead(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            start = time.perf_counter()
            for _ in range(100):
                sa = SharedMemoryArray.allocate_unsafe(manager, shape=(100,), dtype=np.float64)
                sa.close()
    ...
    def test_safe_validates_early_unsafe_validates_late(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            try:
                SharedMemoryArray.allocate(manager, shape=(10, -5), dtype=np.float64)
            ...
            try:
                SharedMemoryArray.allocate_unsafe(manager, shape=(10, -5), dtype=np.float64)
...
class TestSafeVsUnsafeCopy:
    ...
    def test_copy_validates_allocation_unsafe_does_not(self):
        ...
        with SharedMemoryManager() as manager:
            original = np.zeros((10, 20), dtype=np.float64)
    ...
    def test_copy_error_handling_vs_unsafe(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            original = np.zeros((10,), dtype=np.float64)
...
class TestSafeVsUnsafeAttach:
    ...
    def test_attach_validates_shape_and_dtype_unsafe_does_not(self):
        ...
        with SharedMemoryManager() as manager:
            owner = SharedMemoryArray.allocate(manager, shape=(100,), dtype=np.float64)
            name = owner.name

            # Safe attach with correct parameters
            client_safe = SharedMemoryArray.attach(name=name, shape=(100,), dtype=np.float64)
            arr_safe = client_safe.as_array()
            assert arr_safe.shape == (100,)
            client_safe.close()

            # Unsafe attach doesn't validate
            client_unsafe = SharedMemoryArray.attach_unsafe(name=name, shape=(100,), dtype=np.float64)
    ...
    def test_attach_default_owner_false(self):
        ...
        with SharedMemoryManager() as manager:
            owner = SharedMemoryArray.allocate(manager, shape=(10,), dtype=np.int32)
            name = owner.name

            # Attach with default owner
            client = SharedMemoryArray.attach(name=name, shape=(10,), dtype=np.int32)
            assert client.owner is False

            # Unsafe attach also defaults to owner=False
            client_unsafe = SharedMemoryArray.attach_unsafe(name=name, shape=(10,), dtype=np.int32)
...
class TestSafeVsUnsafeAllocateLike:
    ...
    def test_allocate_like_validates_template_unsafe_does_not(self):
        ...
        with SharedMemoryManager() as manager:
            template = np.zeros((50, 60), dtype=np.dtype(np.float32))

            # Safe version validates
            sa_safe = SharedMemoryArray.allocate_like(manager, template)
            assert sa_safe.shape == (50, 60)
            assert sa_safe.dtype == np.dtype(np.float32)
            sa_safe.close()
...
class TestEdgeCasesContextManager:
    ...
    def test_zero_size_array_with_managed_context(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            with SharedMemoryArray.allocate(manager, (0, 10), np.dtype(np.float64)).managed() as sa:
                arr = sa.as_array()
                assert arr.size == 0
    ...
    def test_very_large_array_with_managed_context(self):
        ...
        with SharedMemoryManager() as manager:
            # 10MB array
            with SharedMemoryArray.allocate(manager, (1250000,), np.dtype(np.float64)).managed() as sa:
                arr = sa.as_array()
    ...
    def test_managed_context_non_owner_doesnt_unlink(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            client2 = SharedMemoryArray.attach(name=name, shape=(10,), dtype=np.dtype(np.float64))
    ...
    def test_garbage_collection_with_context_manager(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedMemoryArray.allocate(manager, shape=(100,), dtype=np.dtype(np.float64))
            name = sa.name
            ...
            with pytest.raises(FileNotFoundError):
                SharedMemoryArray.attach(name=name, shape=(100,), dtype=np.dtype(np.float64))
    ...
    def test_context_manager_with_small_arrays(self):
        ...
        with SharedMemoryManager() as manager:
            sizes = [1, 2, 5, 10, 100]
            for size in sizes:
                with SharedMemoryArray.allocate(manager, (size,), np.dtype(np.int32)).managed() as sa:
                    arr = sa.as_array()
                    arr[:] = size
    ...
    def test_managed_wrapper_state_after_close(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedMemoryArray.allocate(manager, shape=(5,), dtype=np.dtype(np.float64))
            managed = sa.managed()
...
class TestSafetyComparison:
    ...
    def test_unsafe_methods_skip_validation_for_performance(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            array = np.ones((100, 200), dtype=np.dtype(np.float32))
