...
class TestValidateShape:
    ...
    def test_valid_tuple_shape(self):
        ...
        assert SharedArray._validate_shape((10, 20)) == (10, 20)
        assert SharedArray._validate_shape((5,)) == (5,)
        assert SharedArray._validate_shape((1, 2, 3, 4)) == (1, 2, 3, 4)

    def test_valid_list_shape(self):
        """Test valid list shapes."""
        assert SharedArray._validate_shape([10, 20]) == (10, 20)
        assert SharedArray._validate_shape([100]) == (100,)

    def test_valid_scalar_shape(self):
        """Test valid scalar shape."""
        assert SharedArray._validate_shape(10) == (10,)

    def test_zero_dimension(self):
        """Test that zero dimensions are allowed."""
        assert SharedArray._validate_shape((0, 10)) == (0, 10)
        assert SharedArray._validate_shape((10, 0)) == (10, 0)

    def test_numpy_integer_shape(self):
        """Test shape with numpy integer types."""
        assert SharedArray._validate_shape((np.int64(10), np.int32(20))) == (10, 20)

    def test_negative_dimension_raises(self):
        """Test that negative dimensions raise ValueError."""
        with pytest.raises(ValueError, match="non-negative"):
            SharedArray._validate_shape((10, -5))
        with pytest.raises(ValueError, match="non-negative"):
            SharedArray._validate_shape((-1,))

    def test_non_integer_dimension_raises(self):
        """Test that non-integer dimensions raise TypeError."""
        with pytest.raises(TypeError, match="must be integers"):
            SharedArray._validate_shape((10.5, 20))
        with pytest.raises(TypeError, match="must be integers"):
            SharedArray._validate_shape(("10", 20))

    def test_non_iterable_non_integer_raises(self):
        """Test that invalid types raise TypeError."""
        with pytest.raises(TypeError, match="must be iterable or integer"):
            SharedArray._validate_shape(None)
        with pytest.raises(TypeError, match="must be iterable or integer"):
            SharedArray._validate_shape("invalid")
...
class TestValidateDtype:
    ...
    def test_valid_string_dtype(self):
        ...
        assert SharedArray._validate_dtype('float64') == np.dtype('float64')
        assert SharedArray._validate_dtype('int32') == np.dtype('int32')
        assert SharedArray._validate_dtype('uint8') == np.dtype('uint8')

    def test_valid_numpy_dtype(self):
        """Test valid numpy dtype objects."""
        assert SharedArray._validate_dtype(np.float32) == np.dtype('float32')
        assert SharedArray._validate_dtype(np.int64) == np.dtype('int64')

    def test_valid_python_type(self):
        """Test valid Python type specifications."""
        assert SharedArray._validate_dtype(float) == np.dtype('float64')
        assert SharedArray._validate_dtype(int) == np.dtype('int64')

    def test_invalid_dtype_raises(self):
        """Test that invalid dtypes raise TypeError."""
        with pytest.raises(TypeError, match="Invalid dtype"):
            SharedArray._validate_dtype("not_a_dtype")
        with pytest.raises(TypeError, match="Invalid dtype"):
            SharedArray._validate_dtype(None)
...
class TestCalculateBufferSize:
    ...
    def test_simple_1d_array(self):
        ...
        assert SharedArray._calculate_buffer_size(shape, dtype) == expected
    ...
    def test_different_dtypes(self):
        ...
        shape = (10,)
        assert SharedArray._calculate_buffer_size(shape, np.dtype('float64')) == 80
        assert SharedArray._calculate_buffer_size(shape, np.dtype('float32')) == 40
        assert SharedArray._calculate_buffer_size(shape, np.dtype('int16')) == 20
        assert SharedArray._calculate_buffer_size(shape, np.dtype('bool')) == 10
...
class TestValidateBufferSize:
    ...
    def test_smaller_size_raises(self):
        ...
        with SharedMemoryManager() as manager:
            shm = manager.SharedMemory(100)  # Too small
            shape = (100,)
            dtype = np.dtype('float64')  # Needs 800 bytes
...
class TestAllocate:
    ...
    def test_allocate_2d_int(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=(10, 20), dtype='int32')
    ...
    def test_allocate_multidimensional(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=(2, 3, 4, 5), dtype='uint8')
    ...
    def test_allocate_with_list_shape(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=[5, 10], dtype=np.float32)
    ...
    def test_allocate_invalid_shape_raises(self):
        ...
        with SharedMemoryManager() as manager:
            with pytest.raises(ValueError, match="non-negative"):
                SharedArray.allocate(manager, shape=(-10, 20), dtype='float64')

            with pytest.raises(TypeError, match="must be integers"):
                SharedArray.allocate(manager, shape=(10.5, 20), dtype='float64')

    def test_allocate_invalid_dtype_raises(self):
        """Test that invalid dtype raises appropriate error."""
        with SharedMemoryManager() as manager:
            with pytest.raises(TypeError, match="Invalid dtype"):
                SharedArray.allocate(manager, shape=(10, 20), dtype='invalid_dtype')

    def test_allocate_zero_size_array(self):
        """Test allocating zero-size array."""
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=(0, 10), dtype='float64')
...
class TestCopy:
    ...
    def test_copy_contiguous_1d_array(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedArray.copy(manager, original)
            copied = sa.as_array()
...
class TestAsArray:
    ...
    def test_as_array_returns_correct_shape(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=(10, 20), dtype='float64')
            arr = sa.as_array()
            assert arr.shape == (10, 20)
    ...
    def test_as_array_returns_correct_dtype(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=(100,), dtype='int32')
            arr = sa.as_array()
            assert arr.dtype == np.dtype('int32')
...
class TestIntegration:
    ...
    def test_allocate_fill_read(self):
        ...
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=(100, 100), dtype='float64')
            arr = sa.as_array()
            arr.fill(3.14)
    ...
    def test_large_array(self):
        ...
        shape = (1000, 1000)
        with SharedMemoryManager() as manager:
            sa = SharedArray.allocate(manager, shape=shape, dtype='float64')
            arr = sa.as_array()
    ...
    def test_multiple_arrays_same_manager(self):
        ...
        with SharedMemoryManager() as manager:
            ...
            sa3 = SharedArray.allocate(manager, shape=(5, 5), dtype='uint8')
